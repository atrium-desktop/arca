...
Lantern maintains its own icon set. Each SVG here is registered with lens
at runtime (`lens_icon_register_svg`, via the lazy table in
`crates/lantern-ui/src/icons.rs`) and drawn through the native lens icon
...
`include_str!` arm in `crates/lantern-ui/src/icons.rs`. Keep glyphs to the
