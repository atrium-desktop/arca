...
This guide establishes the standard workflow for developing Arca alongside
its companion C rendering and UI engine, **Optics**. It uses a dual
...
A feature compiling and passing tests in `arca-dev` is often a **local
...
1. **Local Patch Containment**: `.cargo/config.toml` and path-resolved
   `Cargo.lock` belong strictly to the `arca-dev` worktree. The tracked
...
```text
projects/
├── arca/          # Primary worktree on main (canonical)
├── arca-dev/      # Linked development worktree on dev (local patch)
└── optics/        # Sibling checkout of the Optics C engine
```

### 1. Create the Linked Worktree

Run from the primary `arca` repository (`../arca/`, branch `main`):

```bash
# In projects/arca/ (branch main):
...
```bash
# In projects/arca-dev/ (branch dev):
cd ../arca-dev
...
Run an initial check in `arca-dev` to populate the local worktree
...
All trees must display paths pointing into `../optics/bindings/`.

## Daily Development in Local Mode

> **Active Worktree**: All operations in this section execute inside the
> development worktree (`../arca-dev/`, branch `dev`), interacting with
> the sibling directory `../optics/`.
>
> Do not run these daily development commands in the primary `arca/`
> (`main`) worktree.

### Compiling and Testing

When modifying Optics and Arca simultaneously:
...
1. Recompile the native C libraries whenever Optics C sources change:
   ...
   meson compile -C ../optics/build
   ```
2. Build and test Arca in the development worktree. The `-sys` crates
...
# In projects/arca-dev/ (branch dev):
git add .
git commit -m "feat(chooser): update portal prompter envelope"
```
...
The commit proceeds with only your Arca source changes. Never bypass this
...
Land and tag the required changes in `../optics` before making Arca
canonical:
...
Switch to the primary `arca/` worktree (`branch main`). Because `main`
...
```bash
# In projects/arca/ (primary worktree, branch main):
cd ../arca
git switch main
git pull --ff-only
git merge dev
...
in `projects/arca/Cargo.toml` to the new tag `vX.Y.Z`.
...
Confirm that `cargo tree -i flux` and `cargo tree -i iris` report the tagged
...
Notice that **`arca-dev` never touched or disabled its `.cargo/config.toml`**.
...
```bash
# In projects/arca-dev/ (development worktree, branch dev):
cd ../arca-dev
...
#### 1. In Local Optics Mode (`arca-dev/`)
...
- **Automatic Path Unstaging**: During regular feature commits, the hook
  ...
  ```
  The commit proceeds with only your clean Arca source changes.
- **Refusing Release-Shaped Commits**: If you attempt to commit a version bump
  ...
  directly inside the primary main worktree (../arca/).
  ```

#### 2. In Canonical Mode (`arca/`, `main`)
...
```bash
# In projects/arca/ (main) or projects/arca-dev/ (dev):
...
```bash
# 1. Pull the primary worktree (in projects/arca/, branch main):
cd ../arca
git pull --ff-only

# 2. Rebase the development worktree (in projects/arca-dev/, branch dev):
cd ../arca-dev
git restore Cargo.lock
git rebase main
cargo check --workspace
...
- `tessera-dev`: Wayland compositor and shell.
