    ...
    match pattern.first() {
        ...
        Some(&literal) => {
            if let Some(&nc) = name.first() {
                if nc.eq_ignore_ascii_case(&literal) {
...
pub fn mime_matches(rule: &str, path: &Path) -> bool {
    let mime = crate::mime::guess_mime_type(path);
    if let Some(prefix) = rule.strip_suffix('*') {
        mime.starts_with(prefix)
    } else {
        mime.eq_ignore_ascii_case(rule)
            || crate::mime::default_database().is_subclass_of(&mime, rule)
    }
}
...
mod tests {
    ...
    fn wire_roundtrip() {
        ...
        let prompter_req = PrompterRequest {
            version: PROCESS_CONTRACT_VERSION,
            prompt: PromptRequest::FileChooser(Box::new(req.clone())),
